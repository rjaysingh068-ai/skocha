...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (saved) {
      try {
        ...
        if (parsed.isDeveloper) setActiveTab('admin');
  ...
  const syncProfile = async (agentId: string) => {
    try {
      ...
      if (res.ok) {
        ...
        if (updated.isDeveloper) setActiveTab('admin');
      }
    } catch (e) {
  ...
  const fetchAds = async (cat: string = selectedCategory, loc: string = selectedLocation) => {
    ...
    try {
      ...
      const data = await response.json();
      if (!response.ok) throw new Error(data.error || 'Failed to fetch directory listings.');
      setAds(data);
    } catch (err: any) {
      ...
      <main className="flex-grow pb-16">
        ...
        {activeTab === 'home' && (
          <div>
            ...
            <div className="max-w-7xl mx-auto px-4 lg:px-8 py-8 border-t border-slate-800">
              <div className="text-center mb-10">
                ...
                <p className="text-sm text-slate-400 mt-2 max-w-2xl mx-auto font-light leading-normal">
                  Choose an elite companion service or jump straight to active regional escorts.
                </p>
              </div>
                ...
                {hotspots.map((hs) => (
                  ...
                  >
                    ...
                    <div className="absolute inset-x-0 bottom-0 p-4 z-20 text-left">
                      ...
                      <p className="text-[10px] text-slate-400 leading-snug">{hs.sub}</p>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="max-w-7xl mx-auto px-4 lg:px-8 py-12 mt-4" id="listings-section">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8 border-b border-slate-800 pb-6">
                <div>
                  ...
                  <p className="text-xs text-slate-500 mt-1">Showing {ads.length} active listings in {selectedLocation === 'All' ? 'India' : selectedLocation}.</p>
                </div>
              ...
              {loadingAds ? (
                ...
                <div className="p-16 text-center border border-dashed border-slate-800 rounded-2xl max-w-xl mx-auto">
                  ...
                  <p className="text-xs text-slate-500 mt-1 mb-6">
                    No active ads for {selectedCategory} in {selectedLocation === 'All' ? 'India' : selectedLocation}.
                  </p>
                  {(selectedCategory !== 'All' || selectedLocation !== 'All') && (
                    <button
                      onClick={() => { setSelectedCategory('All'); setSelectedLocation('All'); setSearchLocation('All'); }}
            ...
            <div className="max-w-7xl mx-auto px-4 lg:px-8 py-8 mt-6 border-t border-slate-800">
              <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 md:p-8 flex flex-col md:flex-row items-center justify-between gap-6 relative overflow-hidden shadow-xl">
                ...
                <div className="text-left space-y-2">
                  <span className="text-[10px] font-mono font-bold text-amber-500 uppercase tracking-widest">Help & Customer Support</span>
                  <h3 className="text-xl font-serif font-bold text-white tracking-tight">Need Assistance with Coins or Listings?</h3>
                  <p className="text-xs text-slate-400 max-w-xl">Our dedicated team is available 24/7 to clear pending UPI transaction UTRs and assist with any listing questions.</p>
                </div>
                <div className="flex flex-wrap items-center gap-4 shrink-0">
                  <div className="flex flex-col gap-1.5 text-xs text-slate-300 font-mono text-left mr-2">
                    <div className="flex items-center space-x-2">
                      <span className="text-slate-500">Email:</span>
                      <a href="mailto:supportt.skocha@gmail.com" className="text-amber-400 hover:underline">supportt.skocha@gmail.com</a>
                    </div>
                    <div className="flex items-center space-x-2">
                      <span className="text-slate-500">Phone:</span>
                      <a href="tel:+916378490489" className="text-amber-400 hover:underline">+91 63784 90489</a>
                    </div>
                  </div>
        ...
        {activeTab === 'my-ads' && currentAgent && (
          <div className="max-w-4xl mx-auto p-4 sm:p-6">
            <div className="mb-4 text-left">
              <button onClick={() => setActiveTab('home')} className="inline-flex items-center space-x-1 px-3 py-1.5 bg-slate-900 hover:bg-slate-800 border border-slate-800 text-slate-300 font-bold rounded-lg text-xs font-mono transition-colors">
                <span>← Home</span>
              </button>
            </div>
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
              ...
              <div className="flex items-center space-x-3 p-4 bg-slate-900 border border-slate-800 rounded-xl">
                ...
                <button onClick={() => setShowPaymentModal(true)} className="px-3 py-1.5 bg-amber-500 hover:bg-amber-600 text-slate-950 text-xs font-bold rounded-lg transition-colors">BUY COINS</button>
              </div>
            </div>
            <div className="mb-6">
              ...
              {agentAds.length === 0 ? (
                <div className="p-12 text-center border border-dashed border-slate-800 rounded-2xl">
                  ...
                  <button onClick={() => setActiveTab('post-ad')} className="px-5 py-2 bg-amber-500 hover:bg-amber-600 text-slate-950 font-bold rounded-lg text-xs">Post Your First Ad Listing</button>
                </div>
              ) : (
                <div className="space-y-6">
                  {agentAds.map((ad) => (<AdCard key={ad.id} ad={ad} />))}
                </div>
              )}
            </div>
          </div>
